<template>
  <div>
    ...
    <template v-if="authToken == '' || authToken == null">
      <Login
        class="mx-auto overflow-hidden mt-3 border-0 mainContentWdth"
        :themeData="eventData.orgData"
    ...
    <template v-if="authToken != '' && authToken != null">
      ...
      <div class="footer mx-auto overflow-hidden mainContentWdth" style="align-items: center; padding-top: 20px">
        <b>Disclaimer:</b>
        Anyone can create campaigns on {{ appName }}, rewards are distributed by the campaign creator and
        {{ appName }} is not responsible for reward distribution.
      </div>
    </template>
  </div>
</template>
