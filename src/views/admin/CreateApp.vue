...
<template>
  <div class="home marginLeft marginRight">
        ...
        <div class="card-body">
          <form>
            <div class="row g-3 align-items-center w-100 mt-4">
              <div class="text-left col-lg-3 col-md-3 text-left">
                <label for="appId" class="col-form-label">
                  <tool-tips infoMessage="Application ID, generated once you create an app"></tool-tips>App ID :
                </label>
              </div>
              <div class="col-lg-9 col-md-9 px-0">
                <input disabled v-model="app._id" type="text" id="appId" class="form-control w-100" />
              </div>
            </div>
            <div class="row g-3 align-items-center w-100 mt-4">
              <div class="text-left col-lg-3 col-md-3 text-left">
                <tool-tips infoMessage="Name of the application"></tool-tips
                ><label for="name" class="col-form-label">App Name <span style="color: red">*</span>: </label>
              </div>
              <div class="col-lg-9 col-md-9 px-0">
                <input
                  v-model="app.appName"
            ...
            <div class="row g-3 align-items-center w-100 mt-4">
              <div class="text-left col-lg-3 col-md-3 text-left">
                <tool-tips
                  :infoMessage="`Enter base URL of your server. This URL will be whitelisted to access data from ${appName}`"
                ></tool-tips
                ><label for="baseUrl" class="col-form-label">Base URL <span style="color: red">*</span>: </label>
              </div>
              <div class="col-lg-9 col-md-9 px-0">
                <input
                  v-model="app.baseUrl"
            ...
            <div class="row g-3 align-items-center w-100 mt-4" v-if="isEdit">
              <div class="col-lg-3 col-md-3 text-left">
                <tool-tips infoMessage="Switch to assign new credentials to your app"></tool-tips
                ><label for="endDate" class="col-form-label">Generate Credentials: </label>
              </div>
              <div class="col-lg-9 col-md-9 px-0">
                <b-form-checkbox v-model="app.toggle" name="check-button" switch> </b-form-checkbox>
              </div>
            </div>
          </form>
          <div class="allButtons" style="float: right">
            ...
            <button v-if="isEdit" class="btn btn-light slight-left-margin" type="button" @click.prevent="cancel">
              Cancel
            </button>
          </div>
        </div>
    ...
    <div class="table">
      ...
      <div class="row">
        <div class="col-md-12">
          <table class="table table-bordered" style="background: #ffff" v-if="apps.length">
            ...
            <tbody>
              <tr v-for="row in apps" :key="row._id">
                ...
                <td style="display: flex">
                  <p>{{ truncate1(row.appWalletAddress, 15) }}</p>
                  <i
                    class="far fa-copy"
...
import { isValidURL, isValidText, truncate } from "../../mixins/fieldValidationMixin";
...
export default {
  ...
  methods: {
    ...
    downloadCredentials(credential) {
      this.forceFileDownload(JSON.stringify(credential), "Appcredential.json");
    },
    async generateWallet() {
      ...
      const credential = {
        appWalletAddress: web3.eth.accounts.privateKeyToAccount(BIP32EXTENDED.privateKey.toString("hex"), true).address,
