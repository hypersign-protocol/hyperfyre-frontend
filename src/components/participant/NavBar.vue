...
<template>
  <b-navbar class="nav-bar" :style="themeCss" toggleable="lg" v-if="show">
    ...
    <b-nav-item-dropdown size="sm" right class="m-2 menu ml-auto text-white text-decoration-none">
      ...
      <b-dropdown-item to="/user/home/" @click="updateIsForm"> Home </b-dropdown-item>
      <b-dropdown-item @click="logout" v-if="authToken != '' && authToken != null"> Logout </b-dropdown-item>
    </b-nav-item-dropdown>
  </b-navbar>
</template>
...
export default {
  ...
  computed: {
    themeCss() {
      return {
        "--theme-bg-color":
          this.themeData.themeColor && this.isForm ? this.themeData.themeColor : config.app.themeBgColor,
      };
    },
  },
  data: () => ({
