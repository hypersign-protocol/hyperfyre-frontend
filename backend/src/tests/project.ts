...
describe('API testing /api/v1/project', () => {
  ...
  it('should GET a specific project', (done) => {
    chaiServer.get('/api/v1/project/' + projectData['_id']).end((err, response) => {
      response.should.have.status(200);
      response.body.should.be.a('object');
      response.body.should.have.property('_id').eq(projectData['_id']);
      response.body.should.have.property('projectName').eq('Test New Project');
  ...
  it('should be able to update (PUT) a project', (done) => {
    ...
    chaiServer
      ...
      .end((err, response) => {
        ...
        response.body.should.be.a('object');
        response.body.should.have.property('_id').eq(projectData['_id']);
        response.body.should.have.property('projectName').eq('Test New Project Updated');
        response.body.should.have.property('twitterHandle').eq('hermit1@update');
        response.body.should.have.property('telegramHandle').eq('hermit1@update');
        response.body.should.have.property('ownerDid').eq('did:hs:1231231');
  ...
  it('should DELETE a specific project', (done) => {
    chaiServer.delete('/api/v1/project/' + projectData['_id']).end((err, response) => {
      response.should.have.status(200);
      response.body.should.be.a('object');
      done();
    });
