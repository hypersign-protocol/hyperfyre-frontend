...
async function addUpdateUser(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const { userData, projectId, actions } = req.body;
    ...
    if (actions && actions.length > 0) {
      ...
      actions.forEach((x) => {
        const t: IEventAction = eventActionsInDb.find(
          (y) => y._id == x['actionId'] && x.type != EventActionType.HYPERSIGN_AUTH
        );
    ...
    **/
    ...
    if (investor_email != null) {
      ...
      logger.info(
        'InvestorController:: addInvestor(): after creating a new investor into db id = ' + new_investor['_id']
      );
      ...
      if (isComingFromReferal) {
        ...
        if (investor) {
          ...
          logger.info(
            'InvestorController:: addInvestor(): could not fetch investor. filter = ' + JSON.stringify(filter)
          );
        }
      }
...
async function getCredential(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    logger.info(
      'InvestorController:: getCredential(): before signCredential issuerKeyPair.publicKey.id = ' +
        issuerKeyPair.publicKey.id
    );
    const signedCredential = await hypersignSDK.credential.signCredential(
      rawCredential,
      issuerKeyPair.publicKey.id,
      issuerKeyPair.privateKeyBase58
    );
...
async function sendEmail(data) {
  ...
  await mailService.sendEmail(
    data.email,
    mailTemplateTemp,
    `Congratulations! You are successfully verified with Hypersign`
  );
  return link;
}
...
async function issueCredential(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    req.body['result'] = {
      message: 'Whitelisting credential has been successfully sent to the investor email',
      credentialUrl: link,
    };
    return next();
  } catch (e) {
