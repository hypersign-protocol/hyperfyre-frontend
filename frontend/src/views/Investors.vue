...
<template>
  <div class="home marginLeft marginRight">
    ...
    <!-- <div class="row" style="margin-top: 2%">
      <div class="col-md-12" style="text-align: left">
        <table class="table shadow-sm overflow-hidden  rounded-2 bg-white">
          <thead>
            <tr>
          ...
          <tbody>
            <tr v-for="investor in project.investors" v-bind:key="investor.did">
              ...
              <td style="word-wrap: anywhere;">
                <a
                  :href="'https://etherscan.io/address/' + investor.ethAddress"
                  target="_blank"
                  >{{ investor.ethAddress }}</a
                >
              </td>
              <td>
                <a
                  :href="'https://twitter.com/' + investor.twitterHandle"
                  target="_blank"
                  >@{{ investor.twitterHandle }}</a
                >
              </td>
              <td>
                <a
                  :href="'https://t.me/' + investor.telegramHandle"
                  target="_blank"
                  >@{{ investor.telegramHandle }}</a
                >
              </td>
              ...
              <td>
                ...
                >
                  ...
                  <b-dropdown-item  @click="verifyInvestor(investor)">Verify</b-dropdown-item>
                  <b-dropdown-item  @click="issueCredential(investor)">Issue</b-dropdown-item>
                </b-dropdown>
  ...
  methods: {
    ...
    async handleTableSearch() {
      try {
        ...
        if (this.tableSearch == "") {
          return (this.project = this.sourceData);
        }

        if (!this.investor.projectId) throw new Error("No project found");

        const url = `${this.$config.studioServer.BASE_URL}api/v1/project/search/${this.investor.projectId}?fetchInvestors=true&searchQuery=${this.tableSearch}`;
    ...
    async fetchProjectData(skip, limit) {
      try {
        ...
        const url = `${this.$config.studioServer.BASE_URL}api/v1/project/${this.investor.projectId}?fetchInvestors=true&limit=${limit}&skip=${skip}`;
