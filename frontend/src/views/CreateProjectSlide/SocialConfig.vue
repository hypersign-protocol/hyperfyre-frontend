...
<template>
  <div>
    ...
    <select v-model="selectedSocialMedia" class="form-select form-control">
      <option
        :key="idx"
        v-for="(option, idx) in socialOptions"
        :value="option.value"
      >
        {{ option.label }}
      </option>
    ...
    <div v-if="selectedSocialMedia !== null" class="mt-4">
      <div
        v-for="field in selectedSocialMedia.fields"
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
        ...
        >
          {{
            addedSocialMedias.includes(selectedSocialMedia) ? "Update" : "Add"
          }}
        </button>
      </div>
    </div>
  ...
  methods: {
    removeSocialMedia(index) {
      ...
      this.socialOptions.map((opt) => {
        if (
          opt.value &&
          opt.value.media == this.addedSocialMedias[index].media
        ) {
    ...
    handleSocialMediaAdd(media) {
      ...
      if (!this.addedSocialMedias.includes(media)) {
        if (media.fields[0].value == "" && !media.fields[0].optional) {
          return this.notifyErr(
            `Please fill in ${media.fields[0].placeholder}`
          );
        }

        if (media.fields[1].value == "" && !media.fields[1].optional) {
          return this.notifyErr(
            `Please fill in ${media.fields[1].placeholder}`
          );
        }
        ...
        this.addedSocialMedias[
          this.addedSocialMedias.indexOf(media)
      ...
      this.addedSocialMedias[this.addedSocialMedias.indexOf(media)].fields.map(
        (field) => {
